# django_core_micha/invitations/email_texts.py
...
def get_preferred_language(user) -> str:
    ...
    Du kannst das anpassen (z. B. user.profile.language).
    """
    lang = None
...
def render_pending_registration_email(email, url, language=None):
    ...
    lang = language or getattr(django_settings, "LANGUAGE_CODE", "en")[:2].lower()
    subject_tpl = PENDING_REGISTRATION_SUBJECT.get(
        lang, PENDING_REGISTRATION_SUBJECT["en"]
    )
