...
2.38.0 unblocked on the same day MSG-3c closed and which are cut as **`MSG-3d`** — the WO that closes the
remediation and unblocks MSG-5. §5 records the
...
| MSG-3d | ucm | **the last two blocked rows** (27, 42): unread-reply marker on the thread toggle, and `external_key` surfaced so a host can distinguish managed conversations — without ucm learning jg's vocabulary. Deviation doc reaches zero BLOCKED entries; closes the post-audit remediation and unblocks MSG-5. Envelope `ui-core-micha/work-orders/MSG-3d.md` |
